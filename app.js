...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 🔄 Xử lý sự kiện khi nhấn nút "Swap Now"
swapNowButton.addEventListener("click", async () => {
    ...
    try {
        ...
        // 🔄 Swap từ VIC sang VIN
        if (fromToken === "VIC") {
            const amountInWei = ethers.utils.parseEther(fromAmount.toString());

            console.log(`🚀 Swapping ${fromAmount} VIC to VIN...`);
            tx = await vinSwapContract.swapVicToVin({ value: amountInWei });

        } 
        // 🔄 Swap từ VIN sang VIC
        else {
            const amountInUnits = ethers.utils.parseUnits(fromAmount.toString(), 18);
            const vinContract = new ethers.Contract(vinTokenAddress, vinABI, signer);

            console.log("🔓 Approving swap...");
            const approveTx = await vinContract.approve(vinSwapAddress, amountInUnits);
            await approveTx.wait();
            console.log("✅ Approve successful!");

            console.log(`🚀 Swapping ${fromAmount} VIN to VIC...`);
            tx = await vinSwapContract.swapVinToVic(amountInUnits);
        }

        console.log(`📜 Transaction pending... TX: ${tx.hash}`);
        await tx.wait();
        ...
        alert(`Swap failed! Error: ${error.message || "Please check again."}`);
    }
});
